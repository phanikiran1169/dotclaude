#!/usr/bin/env node
// protect-secrets.js: PreToolUse hook that blocks access to sensitive files and secrets
// protect-secrets.js: Based on karanb192/claude-code-hooks with additional patterns
...
const SENSITIVE_FILES = [
  ...
  { level: 'critical', id: 'gnupg-dir',          regex: /(?:^|\/)\.gnupg\//,                             reason: 'GPG key directory' },
  ...
  { level: 'high', id: 'system-file',            regex: /^\/etc\/(passwd|shadow|hosts|fstab)/,           reason: 'System file' },
...
function checkFilePath(filePath, safetyLevel = SAFETY_LEVEL) {
  ...
  if (!path.isAbsolute(filePath)) {
    return { blocked: true, pattern: { level: 'high', id: 'relative-path', reason: `file path must be absolute: ${filePath}` } };
  }
    ...
    if (result.blocked) {
      ...
      return console.log(JSON.stringify({
        hookSpecificOutput: {
          ...
          permissionDecisionReason: `[${p.id}] Cannot ${action}: ${p.reason}`
        }
      }));
    }
    console.log('{}');
